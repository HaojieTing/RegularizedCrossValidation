...
mxkcv_seq_t_test.task_config_validation <- function(task_config) {
  # 验证序贯检验任务的配置是否正确.
  #
  # Args:
...
mxkcv_seq_t_test.perform_task <- function(task_config) {
  ...
  # 交叉验证类型必须为增量式mx2bcv.
  if(cvConf$name != 'increaseBalancedMx2cv' && cvConf$name != 'mxkrcv_inc') {
...
mxkcv_seq_t_test.write_output <- function(result) {
