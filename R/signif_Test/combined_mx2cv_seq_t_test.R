# 对比两个机器学习算法的Combined mx2 sequential t-test
#
# 检验问题为：
...
combined_mx2cv_seq_t_test.task_config_validation <- function(task_config) {
  # 验证序贯检验任务的配置是否正确.
  #
  # Args:
...
combined_mx2cv_seq_t_test.perform_task <- function(task_config) {
  ...
  for(cur_m in lower_m:(upper_m+agree_cnt)) {
    ...
    # 计算左右边界
    I_l = delta_0 - sqrt(1.0/(2*cvConf$m)*var_est) * qt(p = 1-alpha, f_m_est)
    I_r = delta_1 + sqrt(1.0/(2*cvConf$m)*var_est) * qt(p = 1-beta, f_m_est)
    if(I_r < I_l) {
    ...
    prob_rej_H0 <- pt((mu_diff - delta_0)/(sqrt(var_est)*1.0/(2*cvConf$m)), df = f_m_est, lower.tail = F)
    prob_rej_H1 <- pt((mu_diff - delta_1)/(sqrt(var_est)*1.0/(2*cvConf$m)), df = f_m_est, lower.tail = T)
...
combined_mx2cv_seq_t_test.write_output <- function(result) {
