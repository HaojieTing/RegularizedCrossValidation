...
combined_t_test.process <- function(testConf) {
  ...
  muvec <- sqrt(10) * rowMeans(muv[, 1:(ncol(muv)-1)])
  ...
  y <- 1-pt(const * t_value - sqrt(10) * x_var/(sqrt(mean(var_est_vec))), freedegree)+pt(-const * t_value- sqrt(10) * x_var/(sqrt(mean(var_est_vec))),freedegree)
...
combined_t_test.validate <- function(testConf) {
