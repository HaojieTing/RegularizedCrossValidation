...
uci_nursery.DataGenerator <- function(dataset.conf) {
  data.set <- GetExternalDataSet(foldername = "UCI_ML_DataFolders/nursery",
                                 filename = "nursery.data", sep=",")
...
uci_nursery.PrePackages <- c()

uci_nursery.validation <- function(dataset.conf){
  return(TRUE)
}