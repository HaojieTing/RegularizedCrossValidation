...
uci_optdigits.DataGenerator <- function(dataset.conf) {
  data.set <- GetExternalDataSet(foldername = "UCI_ML_DataFolders/optdigits",
                                 filename = "optdigits", sep=",")
...
uci_optdigits.PrePackages <- c()

uci_optdigits.validation <- function(dataset.conf){
  return(TRUE)
}