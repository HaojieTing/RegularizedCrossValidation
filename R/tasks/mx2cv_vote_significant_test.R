...
mx2cv_vote_significant_test.compute_c_m <- function(m, rho1, rho2) {
  nom <- 1+rho1 + 2.0*(m-1)*rho2
  den <- 2.0*m-nom
  return(sqrt(nom /den))
}

mx2cv_vote_significant_test.compute_c_m_hat14 <- function(m, task_config) {
  return(1)
}

mx2cv_vote_significant_test.compute_f_m_hat14 <- function(m , task_config) {
  return(2*m-1)
}

mx2cv_vote_significant_test.compute_c_m_hat_org <- function(m, task_config) {
  term1 <- 2.0*m+1
  term2 <- 2.0*m-1
...
mx2cv_vote_significant_test.compute_c_m_hat <- function(m, task_config) {
  # 计算序贯检验时所用到的C_m的估计量。
  #
  # Args:
  #   m: 组块mx2中的重复次数m
...
mx2cv_vote_significant_test.compute_c_m_hat_as_one <- function(m, task_config) {
  return(1.0)
}

mx2cv_vote_significant_test.compute_c_m_hat_improve_org <- function(m, task_config) {
  term1 <- 2.0*m+1
  term2 <- 2.0*m-1
  term <- sqrt(term1/term2)
...
mx2cv_vote_significant_test.compute_c_m_hat_improve2_org <- function(m, task_config) {
  term1 <- 2.0*m+1
  term2 <- 2.0*m-1
  term <- sqrt(term1/term2)
  return(0.72*term)
...
mx2cv_vote_significant_test.compute_c_m_hat_improve3_org <- function(m, task_config) {
  term1 <- 2.0*m+1
  term2 <- 2.0*m-1
  term <- sqrt(term1/term2)
  return(0.8*term)
...
mx2cv_vote_significant_test.compute_c_m_hat_improve4_org <- function(m, task_config) {
  term1 <- 2.0*m+1
  term2 <- 2.0*m-1
  term <- sqrt(term1/term2)
  return(0.9*term)
...
mx2cv_vote_significant_test.compute_c_m_hat_improve5_org <- function(m, task_config) {
  rho1 <- 0.25
  rho2 <- 0.4
  nom <- 1+rho1 + 2.0*(m-1)*rho2
  den <- 2.0*m-nom
...
mx2cv_vote_significant_test.compute_c_m_hat_improve6_org <- function(m, task_config) {
  rho1 <- 0.3
  rho2 <- 0.4
  nom <- 1+rho1 + 2.0*(m-1)*rho2
  den <- 2.0*m-nom
...
mx2cv_vote_significant_test.compute_c_m_hat_improve7_org <- function(m, task_config) {
  rho1 <- 0.4
  rho2 <- 0.4
  nom <- 1+rho1 + 2.0*(m-1)*rho2
  den <- 2.0*m-nom
...
mx2cv_vote_significant_test.compute_c_m_hat_improve8_org <- function(m, task_config) {
  rho1 <- 0.0
  rho2 <- 0.5
  nom <- 1+rho1 + 2.0*(m-1)*rho2
  den <- 2.0*m-nom
...
mx2cv_vote_significant_test.compute_f_m_hat_improve8_org <- function(m, task_config) {
  rho1 <- 0.0
  rho2 <- 0.5
...
mx2cv_vote_significant_test.compute_c_m_hat_another <- function(m, task_config) {
  term1 <- 2.0*m/(2.0*m-1)
  term2 <- asin(1.0/(2.0*m)) - asin((1.0-m)/m) - cos(asin(1.0/(2.0*m))) + cos(asin((1.0-m)/m))
  return(term1*term2)
}

mx2cv_vote_significant_test.compute_c_m_hat_empirical <- function(m, task_config) {
...
mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved <- function(m, task_config) {
...
mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v2 <- function(m, task_config) {
...
mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v3 <- function(m, task_config) {
...
mx2cv_vote_significant_test.compute_f_m_hat_empirical <- function(m, task_config) {
  # rho1从0到rho2, rho2从0到1/2
...
mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved <- function(m, task_config) {
  # rho1从1/6到rho2, rho2从1/6到1/2
...
mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved_v2 <- function(m, task_config) {
  # rho1从0到rho2, rho2从1/6到1/2
...
mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved_v3 <- function(m, task_config) {
  # rho1从0到rho2, rho2从1/4到1/2
...
mx2cv_vote_significant_test.compute_f_m_hat <- function(m, task_config) {
  return(2/3*(2*m-1))
}

mx2cv_vote_significant_test.compute_f_m_hat_conv <- function(m, task_config){
  return(m)
}

mx2cv_vote_significant_test.compute_f_m_hat_v2 <- function(m, task_config) {
  return(11/15*(2*m-1))
}

mx2cv_vote_significant_test.compute_f_m_hat_v3 <- function(m, task_config) {
  return(23/36*(2*m-1))
}

mx2cv_vote_significant_test.compute_f_m_hat_v4 <- function(m, task_config) {
  return(1/2.0*(2*m-1))
}


mx2cv_vote_significant_test.est_types <- list(
  "type14est" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat14,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat14
  ),
  "typeI" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeII" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat14
  ...
  "typeIV" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat14
  ),
  "typeV" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_org,
  ...
  "typeVI" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_as_one,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeVII" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeVIII" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve2_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeIX" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve3_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeX" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeXI" = list (
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve4_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "typeXII" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_empirical
  ),
  "type13" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_another,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "type14" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve5_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "type15" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve6_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "type16" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve7_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ),
  "type17" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved
  ),
  "type18" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ...
  "type19" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_v2
  ),
  "type20" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v2,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved_v2
  ),
  "type21" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v2,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat
  ...
  "type22" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v2,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_v3
  ),
  "type23" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_empirical_improved_v3,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_empirical_improved_v3
  ),
  "type24" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_improve8_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_improve8_org
  ),
  "type25" = list(
    "cm" = mx2cv_vote_significant_test.compute_c_m_hat_org,
    "fm" = mx2cv_vote_significant_test.compute_f_m_hat_conv
  )
)


mx2cv_vote_significant_test.task_config_validation <- function(task_config) {
  # 验证序贯检验任务的配置是否正确.
  #
  # Args:
  ...
  if (is.null(task_config$est_type)) {
    ...
    if(!task_config$est_type %in% names(mx2cv_vote_significant_test.est_types))
      stop("Unsupported est type.")
  }
  return(task_config)
}

mx2cv_vote_significant_test.perform_task <- function(task_config) {
  ...
  tmp_agree_idx = 0
  sim_int_result <- c()
  esti_funcs <- mx2cv_vote_significant_test.est_types[[task_config$est_type]]
  ...
  for(cur_m in lower_m:(upper_m+agree_cnt-1)) {
    ...
    # 计算左右边界
    I_l = delta_0 - c_m_est * sqrt(var_est) * qt(p = 1-alpha, f_m_est)
    I_r = delta_1 + c_m_est * sqrt(var_est) * qt(p = 1-beta, f_m_est)
    ...
    type1error <- 0
    type2error <- 0
    if (mu_diff > delta_0 + c_m_est * sqrt(var_est) * qt(p = 1-alpha, f_m_est))
      type1error <- 1
    if(mu_diff < delta_1 - c_m_est * sqrt(var_est) * qt(p = 1-beta, f_m_est))
      type2error <-1
  ...
  colnames(test_result) <- c("m.stop", "test.result","prob.rej.H0","prob.rej.H1", "type1error", "type2error")
  result <- list( "verbose" = verbose.table, "test.result" = test_result, "mu.vec"= mu_diff_vec)
  return(result)
}


mx2cv_vote_significant_test.write_output <- function(result) {
