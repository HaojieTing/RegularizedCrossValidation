...
temp_dir <- "/apps/users/wrb/trunk/papers/neco_yu/table7_letter_signtest_result/b3x2cv_t"

veConf <- list( name = "var_est3_mx2cv",
                data_file = file.path(temp_dir, "sign_test", "diff_mu", fsep = .Platform$file.sep),
                m = 3
)

conf_TASK_SIGN_TEST <- list(algor1_path = file.path(temp_dir, "variance_simulation_classification_tree", "data", 
                                                    fsep = .Platform$file.sep) ,
                            algor2_path = file.path(temp_dir, "variance_simulation_fnn", "data", fsep = .Platform$file.sep) , 
                            diff_mu_file_name = "diff_mu",
                            test_type = "t_test_bmx2cv",
                            alpha = 0.05, 
                            freedegree = 5, 
                            true_mu = 0.0, 
                            const = 1, 
